...
function M.yeet()
	...
	vim.cmd [[startinsert]]
	vim.api.nvim_buf_set_keymap(M.noice_buf, 'i', '<CR>', '<cmd>lua require"custom.noice_rename".yoo()<CR>', {noremap=true, silent=true})
end

function M.setup()
	vim.api.nvim_set_keymap('n', '<F2>', ':lua require"custom.noice_rename".yeet()<CR>', {noremap=true, silent=true})
end
